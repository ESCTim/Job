	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('le_core:logs:playerDied', 1, PlayerId(), _Killer, DeathReason, Weapon)
			else
				TriggerServerEvent('le_core:logs:playerDied', 2, PlayerId(), GetPlayerServerId(Killer), DeathReason, Weapon)
			end
