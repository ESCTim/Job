...
ESX_MENU_LIST.submit = function (namespace, name, data) {
	$.post('https://es_extended/list/menu_submit', JSON.stringify({
...
ESX_MENU_LIST.cancel = function (namespace, name) {
	$.post('https://es_extended/list/menu_cancel', JSON.stringify({
		_namespace: namespace,
		_name: name
	}))
}
...
<div class="main-esx-container" id="menu_default_{{_namespace}}_{{_name}}">
	<div class="main__container">
		<div class="main__header">
			<div class="main__top-header">
				<p>Delarue</p>
			</div>
			<div class="main__top-secondary-header">
				<p>{{{title}}}</p>
			</div>
		</div>

		<div class="main__list-item-container" id="menu_default_{{_namespace}}_{{_name}}_item">
			{{#elements}}
			<div class="main__list-item {{#selected}}active{{/selected}}">
				<div class="main__list-item-left-container">
					<p>{{{label}}}</p>
				</div>

				{{#isSlider}}
				<div class="main__list-item-right-container">
					<div class="main__list-item-right-left-arrow">
						<img src="img/left_arrow.svg" alt="">
					</div>
					<div class="main__list-item-right-polygon">
						<div class="main__list-item-right-polygon-border">
							<img src="img/pol_6.svg" alt="">
							<img src="img/polygon_inner.svg" alt="">
							<div class="main__list-item-right-polygon-text">
								<p>{{sliderLabel}}</p>
							</div>
						</div>
					</div>
					<div class="main__list-item-right-right-arrow">
						<img src="img/right_arrow.svg" alt="">
					</div>
				</div>
				{{/isSlider}}

				<div class="main__item-bg-sphere-1"></div>
				<div class="main__item-bg-sphere-2"></div>
			</div>
			{{/elements}}
		</div>

		<div class="main__container-bg-circular">
			<div class="main__container-bg-sphere-1"></div>
			<div class="main__container-bg-sphere-2"></div>
			<div class="main__container-bg-sphere-3"></div>
		</div>
	</div>
</div>
`
...
ESX_MENU_DEFAULT.open = function (namespace, name, data) {
	...
	$('#menu_default_' + namespace + '_' + name + '_item').scrollTop($('#menu_default_' + namespace + '_' + name + '_item').find(".main__list-item.active").offset().top - $('#menu_default_' + namespace + '_' + name + '_item').find(".main__list-item").offset().top)
}
...
ESX_MENU_DEFAULT.submit = function (namespace, name, data) {
	$.post('https://es_extended/default/menu_submit', JSON.stringify({
		_namespace: namespace,
		_name: name,
		current: data,
...
ESX_MENU_DEFAULT.cancel = function (namespace, name) {
	$.post('https://es_extended/default/menu_cancel', JSON.stringify({
		_namespace: namespace,
		_name: name
	}))
}
...
ESX_MENU_DEFAULT.change = function (namespace, name, data) {
	$.post('https://es_extended/default/menu_change', JSON.stringify({
		_namespace: namespace,
		_name: name,
		current: data,
		elements: ESX_MENU_DEFAULT.opened[namespace][name].elements
...
ESX_MENU_DIALOG.open = function (namespace, name, data) {
	...
	// 		if (key.which == 27) {
	// 			$.post("https://es_extended/dialog/menu_cancel", JSON.stringify(data))
	// 		} else if (key.which == 13) {
	// 			$.post("https//es_extended/dialog/menu_submit", JSON.stringify(data))
	// 		}
	// 	}
	// }
...
ESX_MENU_DIALOG.change = function (namespace, name, data) {
	$.post("https://es_extended/dialog/menu_change", JSON.stringify(data))
}
						...
						switch (event.data.control) {
							...
							case 'TOP': {
								...
								if (typeof focused != 'undefined') {
									...
									$('#menu_default_' + focused.namespace + '_' + focused.name + '_item').scrollTop($('#menu_default_' + focused.namespace + '_' + focused.name + '_item').find(".main__list-item.active").offset().top - $('#menu_default_' + focused.namespace + '_' + focused.name + '_item').find(".main__list-item").offset().top)
								}
								break
							}
							case 'DOWN': {
			...
			case 27:
				switch (currentScript) {
					case 'list':
						...
						break
					case 'dialog':
						$.post("https://es_extended/dialog/menu_cancel", JSON.stringify(ESX_MENU_DIALOG.DATA))
						break
				}
				break
			case 13:
				switch (currentScript) {
					case 'dialog':
						$.post("https://es_extended/dialog/menu_submit", JSON.stringify(ESX_MENU_DIALOG.DATA))
						break
				}
				break
		}
